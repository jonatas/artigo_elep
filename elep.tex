...
\lstset{language=ruby,caption=Código Ruby,label=Ruby, numbers=left, frame=single} 
...
\subsection { Simplicidade } 
   Os domínios específicos tornam o código estético e preciso. Matz, o criador da linguagem ruby, disse em uma entrevista a Dave Thomas que não é possível criar uma linguagem perfeita, ...... do código, colaboram para de declaração de elementos \cite{programmingRuby}

\subsection { Experiência no domínio específico }:
 Heuert conclui que quanto mais conhecido é o domínio específico, melhor será a solução para este domínio.
\subsection { Metaprogramação } permite estender a linguagem a partir da própria linguagem.
 Dave Thomas ressalta que uma das coisas que mais chama atenção em Ruby é a facilidade de instrospecção a linguagem. 
\end{itemize}
